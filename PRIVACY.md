...
- **No tracking** — no analytics, no cookies, no fingerprinting, no fonts or CDNs, zero external requests. The only bundled third-party code is two open-source MIT libraries (EXIF parsing and QR generation) that are **self-hosted** in `vendor/` and run entirely on your device — nothing is fetched from anywhere.
